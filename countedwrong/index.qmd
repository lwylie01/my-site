---
title: "The Maturity Gap"
subtitle: "Young adults in the legal system, one chart at a time"
format:
  html:
    toc: true
    toc-depth: 2
...
system, then picks the chart built to answer it. The questions add up to
one story. The charts add up to a toolbox.

## The Frame {.section-heading}

What does the field believe happens between 12 and 30? Supports build,
risks fade, and arrests rise and fall across the same window. I drew the
two curves together: the arc of growing up laid over the curve of
desistance.
...
Is the famous curve real? Here it is in arrest data: robbery arrest rates
by age, drawn for three different years.
...
lines would bury the story in spaghetti.</p>
:::

## Everywhere in the System {.section-heading}

How much of the system is this one age band? Ages 18 to 24 were about one
in ten Americans. Then count them at each stage.
...
Who comes back? Follow everyone released from state prison in 2005 for
nine years, and split them by how old they were on release day.
...
The charts above count young adults from the outside. To watch growing up
happen from the inside, you need the same people measured again and
again. That is Pathways to Desistance
([ICPSR study 29961](https://www.icpsr.umich.edu/web/ICPSR/studies/29961)):
young people enrolled in the early 2000s shortly after each was found
responsible for a serious offense, then tracked across 11 interview waves
over seven years.
...
all. Four numbers with nothing to compare are a sentence wearing a chart
costume, so give each one a tile and its own weight.</p>
:::
...
Does maturity finish at 18? Line up all of the interviews by the age the
person was when they gave one, and take the mean at each age.
...
observed climb lands on the adult side of the line.</p>
...
Do the subscales tell different stories? Almost: identity barely moves
before 18, then does most of its growing after. But nothing flattens at
the line.
...
is the whole trick; it only works because these four measures share a
scale.</p>
:::

## What Arrives After 18 {.section-heading}

Maturity is not one thing. The study also measured the capacities the
law's line is supposed to stand for: impulse control, temperance, future
outlook, and consideration of others.
...
Means are tidy. People are not. Drop the averaging: each column is an
age, and each cell shows the share of that age's interviews landing in
one half-point score bin.
...
group cross the line. At the first interview, 92 percent of the cohort
was under 18. Four years in, no one was.
...
ggplotly_titled(p_area, tooltip = "text") |> clear_bg()
```
...
Put the two halves of the story in one frame. As this cohort aged from 15
to 24, the share who reported any offending fell. Their measured maturity
rose. The law's line crosses both curves mid-climb.
...
#| out-width: "100%"
gap_off <- offending |>
  transmute(age, value = share_any, n,
            panel = "Share reporting any offending")
gap_mat <- psmi |>
...
p_gap <- ggplot(gap, aes(x = age, y = value)) +
  ...
  geom_point(aes(color = panel, text = paste0(
    "<b>Age ", age, "</b><br>",
    panel, ": ",
    ifelse(panel == "Share reporting any offending",
           percent(value, accuracy = 0.1), sprintf("%.2f", value)),
    "<br>Interviews: ", format(n, big.mark = ",")
  )), size = 1.9) +
  scale_color_manual(values = c(
    "Share reporting any offending" = coral_dark,
...
designer create any crossing point they like by stretching either scale;
two panels make the same comparison without the lie.</p>
:::
